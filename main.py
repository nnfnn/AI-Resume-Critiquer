...
def calculate_ats_score(sections, keywords):
    score = 0
    weights = {
        "Skills": 20, "Experience": 20, "Education": 20,
...
3. WHAT NEEDS WORK
   3 honest weaknesses. Do not sugarcoat — be direct but kind.
...
with col2:
    ...
    2. **Tell us** the role you are targeting
...
if analyze_btn:
    ...
    if not job_role:
        st.warning("Please tell us what role you are applying for.")
        st.stop()
    ...
    with st.spinner("Reading your resume and thinking hard... give us a moment."):
        ...
        if not resume_text:
            st.error("We could not read text from this PDF. Try a different file.")
            st.stop()
    ...
    m4.metric("Pages Scanned",
              f"{len(PyPDF2.PdfReader(io.BytesIO(uploaded_file.getvalue())).pages)} pages")
